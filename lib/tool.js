...
export class CdsTool {
    ...
    adjustManifestJson() {
      ...
      console.log('----- Adjustments of Manifest.json files(s) -----')
      ...
      for (const jsonFileName of this.findManifestJsonFiles(this.runDirectory)){
        ...
        if (!jsonFile['sap.cloud']) {
          console.warn('sap.cloud section does not exist in manifest.json. Adding it now.')
          jsonFile['sap.cloud'] = {
            "public": true,
            "service" :this.appNameSpace
          }
          console.log('The following "sap.cloud" section was added to this "manifest.json":')
          console.dir(jsonFile['sap.cloud'], {depth: null, colors: true})

        } else {
          if (!jsonFile['sap.cloud']['public']) {
            console.warn('Public parameter in sap.cloud section does not exist. Adding it now.')
    ...
    adjustMtaYaml() {
        ...
        if (!this.doesMtaYamlExist()) {
            console.error('mta.yaml does not exist in directory: ', this.runDirectory)
            return
        }

        console.log('----- Adjustments of mta.yaml file -----')
...
*/
        ...
        console.warn('Assuming content deployment step does not exist in mta.yaml. Adding it now.')
        ...
        const DESTINATION_CONTENT_DEPLOYMENT_STEP = [
          ...
          console.warn('Destination Deployment step added:')
